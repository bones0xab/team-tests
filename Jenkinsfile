pipeline {
                    ...
                    def buildResult = sh(
                        script: """
                            docker build \
                                ...
                                -t ${IMAGE_NAME}:feature-nour-latest \
                                .
                        """,
                        returnStatus: true
                    )
        ...
        stage('🤖 Test Agents') {
            ...
            steps {
                ...
                script {
                    ...
                    if (!ollamaUp) {
                        echo '⏭️ Ollama not reachable — skipping agent tests'
                    ...
                    if (v1Result == 0) {
                        ...
                        echo '⚠️ AgentV1 test failed (non-blocking)'
                    ...
                    if (v2Result == 0) {
                        ...
                        echo '⚠️ AgentV2 test failed (non-blocking)'
                        currentBuild.result = 'UNSTABLE'
                    }
                }
            }
    ...
    post {
        always {
            echo '🧹 Cleanup...'
            sh '''
                # Remove test/smoke containers
                docker ps -a | grep -E "(test-|smoke-)" | awk '{print $1}' | xargs -r docker rm -f || true
                
                # Keep only last 5 feature-nour images
                docker images | grep "feature-nour" | tail -n +6 | awk '{print $3}' | xargs -r docker rmi -f || true
        ...
        success {
            script {
                if (env.BRANCH_NAME == 'feature/nour') {
                    echo """
╔════════════════════════════════════════════════════╗
...
╚════════════════════════════════════════════════════╝
                    ...
                    echo "✅ Build successful (branch: ${env.BRANCH_NAME})"
                }
            }
        }
        unstable {
...
║  🧪 Unit tests passed                              ║
║  ⚠️  Some agent tests failed (non-blocking)       ║
